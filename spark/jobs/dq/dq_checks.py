...
class BaseDQChecks(ABC):
    ...
    def _run_deequ_verification(self, df: DataFrame, check: "Check") -> List[DQCheckResult]:
        """
    ...
    def _compute_metric_from_constraint(
        self, df: DataFrame, constraint: str, row_count: int
    ) -> Optional[float]:
        """
        ...
        try:
            ...
            if "Compliance" in constraint or "ContainedIn" in constraint:
                ...
                match = re.search(
                    r"`(\w+)`\s+(?:IS NULL OR\s+)?`?\1?`?\s*IN\s*\(([^)]+)\)", constraint
                )
                if match:
                    col_name = match.group(1)
            ...
            if "NonNegative" in constraint:
                ...
                if match:
                    col_name = match.group(1)
                    valid_count = df.filter(col(col_name).isNull() | (col(col_name) >= 0)).count()
                    return valid_count / row_count
    ...
    def _infer_check_type(self, constraint: str) -> CheckType:
        ...
        if "completeness" in constraint_lower or "isnull" in constraint_lower:
            ...
            "nonnegative" in constraint_lower
            or "maximum" in constraint_lower
            or "minimum" in constraint_lower
        ):
            return CheckType.VALIDITY
    ...
    def _check_completeness_pyspark(
        self, df: DataFrame, column: str, threshold: float = 1.0, is_critical: bool = True
    ) -> DQCheckResult:
        """
...
class BronzeDQChecks(BaseDQChecks):
    ...
    def _check_timeliness(self, df: DataFrame) -> DQCheckResult:
        ...
        try:
            ...
            timed_df = df.filter(
                col("event_timestamp").isNotNull() & col("bronze_processed_at").isNotNull()
            )
            ...
            timed_df = timed_df.withColumn(
                "latency_seconds",
                spark_abs(
                    unix_timestamp("bronze_processed_at") - unix_timestamp("event_timestamp")
                ),
            )
    ...
    def _check_anonymous_accuracy(self, df: DataFrame) -> DQCheckResult:
        ...
        try:
            ...
            user_df = user_df.withColumn(
                "expected_anonymous",
                when(
                    regexp_extract(col("user_normalized"), self.IP_PATTERN, 0) != "", lit(True)
                ).otherwise(lit(False)),
            )
...
class GoldDQChecks(BaseDQChecks):
    ...
    def run_all_checks(
        self, df: DataFrame, run_id: str, bronze_passed: bool = True, silver_passed: bool = True
    ) -> DQGateResult:
    ...
    def _check_upstream_gates(self, bronze_passed: bool, silver_passed: bool) -> DQCheckResult:
        """Check that upstream gates passed."""
        evidence = {"bronze_gate_passed": bronze_passed, "silver_gate_passed": silver_passed}
    ...
    def _check_events_users_consistency(self, df: DataFrame) -> DQCheckResult:
        ...
        try:
            check_df = df.filter(col("total_events").isNotNull() & col("unique_users").isNotNull())
            ...
            violation_count = check_df.filter(col("total_events") < col("unique_users")).count()
