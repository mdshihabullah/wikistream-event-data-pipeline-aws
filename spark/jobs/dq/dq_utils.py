...
class DQAuditWriter:
    ...
    def write_results(self, gate_result: DQGateResult) -> None:
        ...
        for check in gate_result.checks:
            row = Row(
                ...
                threshold_value=float(check.threshold_value)
                if check.threshold_value is not None
                else None,
    ...
    def get_recent_gate_status(self, layer: str, lookback_hours: int = 1) -> Optional[bool]:
        """
...
class DataProfiler:
    ...
    def profile_column(
        self, df: DataFrame, column_name: str, is_numeric: bool = False
    ) -> ColumnProfile:
        """
        ...
        if is_numeric:
            try:
                ...
                profile.percentile_25 = (
                    float(percentiles["p25"]) if percentiles["p25"] is not None else None
                )
                profile.percentile_50 = (
                    float(percentiles["p50"]) if percentiles["p50"] is not None else None
                )
                profile.percentile_75 = (
                    float(percentiles["p75"]) if percentiles["p75"] is not None else None
                )
                profile.percentile_95 = (
                    float(percentiles["p95"]) if percentiles["p95"] is not None else None
                )
            except Exception as e:
    ...
    def profile_dataframe(
        self, df: DataFrame, columns: List[str], numeric_columns: Optional[List[str]] = None
    ...
    def save_profiles(
        self, profiles: Dict[str, ColumnProfile], run_id: str, layer: str, table_name: str
    ) -> None:
    ...
    def get_baseline_profile(
        self, layer: str, table_name: str, column_name: str, lookback_days: int = 7
                ...
                metrics.append(
                    {
                        ...
                        "Value": pass_rate,
                        "Unit": "Percent",
                        "Dimensions": [{"Name": "Layer", "Value": gate_result.layer}],
                    }
                )
